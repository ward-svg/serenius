...
export default function TenantProvider({ config, children }: Props) {
  ...
  useEffect(() => {
    ...
    root.style.setProperty("--button-primary-bg", branding.primary_color);

    root.style.setProperty("--button-primary-text", "#ffffff");

    root.style.setProperty("--button-ghost-border", branding.primary_color);

    root.style.setProperty("--button-ghost-text", branding.primary_color);

    root.style.setProperty("--link-action-color", branding.accent_color);

    root.style.setProperty(
      "--table-header-bg",
      `${branding.secondary_color}22`,
    );

    root.style.setProperty(
      "--table-row-hover-bg",
      `${branding.secondary_color}14`,
    );
