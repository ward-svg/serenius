...
## 6b. Recipient Preview Modal (Implemented)
...
A summary chip row at the top shows all classified counts:
- `N Eligible` (green) — contacts that will receive this send
- `N Suppressed` (amber) — in-segment, version-allowed, but suppressed
- `N Version skipped` (gray) — in-segment but campaign_version filter excluded them
- `N Missing email` (gray) — in-segment and version-allowed but no primary_email

Below the chips, a table lists all eligible contacts: Name, Email, Version, Status. Excluded categories are summarized in a note block below the table with plain-text explanations.
...
## 6c. Campaign Duplication / Clone (Implemented)
...
**Story block body format:** Story/Text blocks support two body format modes via the `bodyFormat` field (`'plain' | 'basic_html'`, optional, defaults to `'plain'`). In **Plain text** mode (default), content is HTML-escaped and double line breaks become `<p>` paragraphs; single line breaks become `<br>`. This is unchanged behavior for all existing blocks. In **Basic HTML** mode, the editor switches to a `StoryFormattedEditor` with two views — **Formatted** (default) and **HTML** — toggled by a tab strip. The **Formatted** view is a `contentEditable` div with a four-button toolbar (Bold, Italic, Underline, Link); Enter creates a new paragraph, Shift+Enter inserts a line break. The toolbar uses `document.execCommand` (deprecated but V1-acceptable for this narrow scope). The Link button uses `window.prompt` to collect a URL; only `https://`, `http://`, `mailto:`, `tel:`, `/`, and `#` prefixes are accepted. `onInput` normalizes `contentEditable` output via `normalizeEditorHtml` (converts div→p wrappers, strips span/class/style/id attributes, preserves href). The **HTML** view shows a raw textarea for power users. Switching from HTML → Formatted syncs the latest HTML into the editor DOM. The renderer's `sanitizeBasicHtml` whitelist runs at render time regardless of which view was used: allowed tags are `<strong>`, `<b>`, `<em>`, `<i>`, `<u>`, `<br>`, `<p>`, `<a href>`. Disallowed tags removed or escaped. Personalization tokens (`{firstname}` etc.) pass through both modes and both views unchanged. No new dependencies added; all helpers in `BlockComposer.tsx` and `email-builder-renderer.ts`.
...
## 7b. Opt-Outs Tab (Communications → Opt-Outs)

### Navigation

Tab order: **Campaigns | Templates | Image Gallery | Brand Kit | Opt-Outs | Delivery Setup**

`CommWorkspaceTab` union includes `'opt-outs'`.

### Opt-out persistence — campaign attribution

When a recipient redeems an opt-out token at `/mail/preferences/[token]`, the server now reads `partner_email_id` from `email_opt_out_tokens` and includes it in the `partner_email_suppressions` insert:

```
partner_email_suppressions.partner_email_id = token.partner_email_id ?? null
```

Campaign-attributed tokens (generated by `campaign-live-send`) carry `partner_email_id = campaign.id`. Tokens generated by other flows (test sends, future notification emails) may have `partner_email_id = null` — that is correct and handled. The suppression row is always written regardless.

Older suppression records created before this slice will have `partner_email_id = null` and will show "—" in the Campaign column of the Opt-Outs grid.

### Active suppression filtering

All pre-send suppression checks use `restored_at IS NULL` to consider only active suppressions:
- `campaign-live-send` route: `.is('restored_at', null)` on suppression query
- `CampaignModal` `estimate` and `recipientPreviewData` memos: `.filter((s) => !s.restored_at)` before building suppressed email set

Restored rows remain in history but no longer block sends.

### Opt-Outs grid (OptOutsTab component)

Loaded lazily on tab mount via browser Supabase client. Scoped to `tenant_id`. Limit 100, newest first (`suppressed_at DESC`). Joins `partner_contacts(display_name)` and `partner_emails(subject)` via FK relationships. All rows fetched; filtering is client-side.

**Filter tabs:** Active (default) · Restored · All

| Column | Source |
|---|---|
| Name | `partner_contacts.display_name` (null → "—") |
| Email | `partner_email_suppressions.email` |
| Campaign | `partner_emails.subject` (null partner_email_id → "—") |
| Type | `suppression_type` — Unsubscribed · Bounced · Complaint · Manual · Invalid Email |
| Source | `source` — Email link · Bounce · Manual · Campaign (null → "—") |
| Opted Out | `suppressed_at` formatted MM/DD/YYYY h:mm AM/PM |
| Status | Active (green badge) or Restored (blue badge + restored date + restore reason) |
| Actions | "Restore" button — Active rows only, `canManage` users only |

### Restore Email workflow

Entry: "Restore" button on Active rows (canManage only).

Modal requires a non-empty **Restore reason / confirmation note** before the confirm button is enabled. Warning copy: "This will allow this email address to receive future campaign emails again. Only restore this address if the recipient personally requested it or confirmed the opt-out was accidental."

On confirm: `partner_email_suppressions` row updated — `restored_at = now()`, `restored_by = auth user id`, `restore_reason = note`. Row is NOT deleted.

On success: local grid row updated immediately. In Active filter view the row disappears; in All view it shows as Restored with the date and reason visible.

If update fails: inline error shown, no optimistic state change.
