...
**Builder block font sizes:** All nine font-size number inputs in `BlockComposer` (header tagline, hero eyebrow/headline/subtitle, story text, highlight heading/body, CTA heading/body) allow `min=2` `max=120` to support creative use cases such as captions, labels, fine print, and decorative micro-copy. Steps and default values are unchanged. The required compliance footer font size remains guarded at `min=11` `max=16` in Brand Kit — that guardrail is enforced by the DB CHECK constraint and `BrandKitTab` validation and is entirely separate from builder block controls.
